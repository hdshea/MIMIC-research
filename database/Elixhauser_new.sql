.print 'Creating ELIXHAUSER_AHRQ view...'

DROP VIEW IF EXISTS ELIXHAUSER_AHRQ;
CREATE VIEW ELIXHAUSER_AHRQ as
with
icd as
(
  ...
  and   seq_num != 1 -- we do not include the primary icd-9 code
)
,
eliflg as
(
...
from
(
  select hadm_id, subject_id, drg_type, cast(drg_code as numeric) as drg_code from drgcodes
    where subject_id in (
        10006, 10011, 10013, 10017, 10019, 10026, 10027, 10029, 10032, 10033,
        10035, 10036, 10038, 10040, 10042, 10043, 10044, 10045, 10046, 10056,
        10059, 10061, 10064, 10065, 10067, 10069, 10074, 10076, 10083, 10088,
