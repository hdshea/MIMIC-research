...
db_get_datetimeevents <- function(con, ...) {
    db_get_from_table(con, "datetimeevents", ...)  %>%
        mutate(
            VALUE = ifelse(str_length(VALUE) == 10, str_c(VALUE," 00:00:00"), VALUE),
            VALUE = ymd_hms(VALUE)
...
db_get_patients <- function(con, ...) {
    db_get_from_table(con, "patients", ...)  %>%
        mutate(
            DOB = ifelse(str_length(DOB) == 10, str_c(DOB," 00:00:00"), DOB),
            DOB = ymd_hms(DOB),
            DOD = ifelse(str_length(DOD) == 10, str_c(DOD," 00:00:00"), DOD),
            DOD = ymd_hms(DOD),
            DOD_HOSP = ifelse(str_length(DOD_HOSP) == 10, str_c(DOD_HOSP," 00:00:00"), DOD_HOSP),
            DOD_HOSP = ymd_hms(DOD_HOSP),
            DOD_SSN = ifelse(str_length(DOD_SSN) == 10, str_c(DOD_SSN," 00:00:00"), DOD_SSN),
